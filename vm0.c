...
void init_tasks(another_vm *vm) {
  for (int i=0; i<64; i++) {
    vm->tasks[i].pc = i==0? 0 : -1;
    vm->tasks[i].next_pc = -1;
    vm->tasks[i].state = 0;
    vm->tasks[i].next_state = 0;
  }
}
...
#define F8  (code[vm->tasks[current_task].pc++])
#define F16 (F8 << 8 | F8)
void tick(another_vm *vm, u8 *pal, u8 *code, u8 *polygons1, u8 *polygons2) {

  printf("[tick: %u, pc=0x%04x] ", vm->ticks, vm->tasks[current_task].pc);
  u8 op = F8;
  printf("op=0x%02x ", op);

  sprintf(debug_msg, "[%u, pc=0x%04x, op=0x%02x]", vm->ticks, vm->tasks[current_task].pc, op);
  u8 arg0, arg1;
  u16 arg;
    ...
    case 0x00 ... 0x1f:
      switch (op) {
        ...
        case 0x4:
          ...
          arg = F16;
          printf("CALL 0x%04x\n", arg);
          vm->tasks[current_task].stack[vm->tasks[current_task].stack_ptr++] = vm->tasks[current_task].pc;
          vm->tasks[current_task].pc = arg;
          break;
        case 0x5:
          // RETURN
          vm->tasks[current_task].pc = vm->tasks[current_task].stack[vm->tasks[current_task].stack_ptr-1]; vm->tasks[current_task].stack_ptr--;
          printf("RETURN 0x%04x\n", vm->tasks[current_task].pc);
          break;
        case 0x6:
          // yield
          vm->task_paused = 1;
        ...
        case 0x9:
          ...
          vm->regs[arg0] -= 1;
          arg = F16;
          printf("jump nz 0x%04x var[%d]=%d\n", arg, arg0, vm->regs[arg0]);
          if (vm->regs[arg0] != 0) {
            vm->tasks[current_task].pc = arg;
          }
          break;
