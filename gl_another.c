...
void texupdate() {
  ...
  //for (int i=0; i<TEX_W*TEX_H; i++) data_real[i] = data_real[tex_no][i];
  glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, TEX_W, TEX_H, GL_RED, GL_FLOAT, data_real );
...
int main(int argc, char **argv) {
  ...
  for (int i=0; i<NUM_BUFFERS; i++) {
    ...
    for (int j=0; j<TEX_W*TEX_H; j++) data_real[j] = (rand() / (float)RAND_MAX + 1.0f);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, TEX_W, TEX_H, 0, GL_RED, GL_FLOAT, data_real);
